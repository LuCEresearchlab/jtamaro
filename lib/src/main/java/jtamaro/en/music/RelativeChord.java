...
import static jtamaro.en.music.Scales.C_MAJOR;
...
/**
 * A chord like they are used in chord progressions,
 * with a scale step (usually shown as a Roman numeral)
 * and an underlying chord (e.g., a Major triad).
 * 
 * TODO: Not at all sure this is right.
 * 
 * @see <a href="https://en.wikipedia.org/wiki/Chord_(music)#Roman_numerals">Wikipedia: Chord</a>
 * @see <a href="https://en.wikipedia.org/wiki/Roman_numeral_analysis">Wikipedia: Roman numeral analysis</a>
 */
public record RelativeChord(int scaleStep, Chord chord, String symbol) {
  ...
  private static void play(Scale  scale, Sequence<RelativeChord> cs) {
    Sequence<AbsoluteChord> acs = map(rc -> scale.in(rc), cs);
    playChords(acs, 60);
  }
